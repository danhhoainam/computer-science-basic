/**
 * DFS use pre-order traversal
 * 
 * A depth-first search, or DFS, is the strategy that goes down a branch all the way 
 * until a leaf is reached, processes said branch, and then moves on to a different branch. 
 * This type of algorithm generally uses a stack in order to keep track of visited nodes, 
 * as the last node seen is the next one to be visited and the rest are stored to be visited later.
 * 
